...
class Date extends Color
{
    ...
    {
        if (!in_array($type, ['date', 'datetime-local', 'month', 'time', 'week'])) {
    ...
    {
        return <<<'str'
<input type="{$type}" name="{$name}" value="{$value}" title="{$title??''}" style="{$style??''}" min="{$min??''}" max="{$max??''}" step="{$step??''}" maxlength="{$maxlength??''}" <?php if (isset($required) && $required) { ?> required<?php } ?><?php if (isset($disabled) && $disabled) { ?> disabled<?php } ?><?php if (isset($readonly) && $readonly) { ?> readonly<?php } ?><?php if (isset($autofocus) && $autofocus) { ?> autofocus<?php } ?><?php if (isset($autocomplete) && !$autocomplete) { ?> autocomplete="off"<?php }else{ ?> autocomplete="on"<?php } ?>>
str;
    }
}
