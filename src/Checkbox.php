...
class Checkbox implements Stringable
{
    ...
    public function __toString()
    {
        $tpl = <<<'str'
<label>
    <input type="checkbox" name="{$name}" value="{$value}" title="{$title??''}" style="{$style??''}" <?php if (isset($required) && $required) { ?> required<?php } ?><?php if (isset($disabled) && $disabled) { ?> disabled<?php } ?><?php if (isset($readonly) && $readonly) { ?> readonly<?php } ?><?php if (isset($autofocus) && $autofocus) { ?> autofocus<?php } ?><?php if (isset($checked) && $checked) { ?> checked<?php } ?>>
    <span>{$label}</span>
</label>
str;
